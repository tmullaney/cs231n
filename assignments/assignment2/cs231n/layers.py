...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    #############################################################################
    # mean/var for each dimension over batch
    ...
    ivar = 1. / sqrtvar
    out_hat = xmu * ivar # (N, D)

    # scale and shift
    out = gamma * out_hat + beta

    # store intermediate values
    cache = (gamma, out_hat, ivar, sqrtvar, xmu, sample_var, eps)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
  elif mode == 'test':
    ...
    # scale and shift
    out = gamma * out_hat + beta # (D,) * (N,D) + (D,)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
  else:
...
def batchnorm_backward(dout, cache):
  ...
  N = dout.shape[0]
  D = dout.shape[1]
  gamma, out_hat, ivar, sqrtvar, xmu, sample_var, eps = cache

  # backprop
  dbeta = np.sum(dout, axis=0) # (D,)
  dgamma = np.sum(dout * out_hat, axis=0) # (D,)

  dout_hat = dout * gamma # (N,D) * (D,) = (N,D)
  dxmu1 = dout_hat * ivar # (N,D)

  divar = np.sum(dout_hat * xmu, axis=0) # (D,)
  dsqrtvar = divar * (-1.) / (sqrtvar ** 2) # (D,)
  dvar = dsqrtvar * 0.5 / sqrtvar
  dsq = dvar * np.ones((N,D)) * (1./N)
  dxmu2 = dsq * 2. * xmu
  dmu = -np.sum(dxmu1 + dxmu2, axis=0) # (D,)
  dx2 = dmu * np.ones((N,D)) * (1./N) # (N,D)
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
  ...
  #############################################################################
  N, C, H, W = x.shape
  x_trans = x.reshape(N, -1)
  gamma_trans = np.tile(gamma, H*W)
  beta_trans = np.tile(beta, H*W)
  out_trans, cache = batchnorm_forward(x_trans, gamma_trans, beta_trans, bn_param)
  out = out_trans.reshape(x.shape)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def spatial_batchnorm_backward(dout, cache):
  ...
  #############################################################################
  N, C, _, _ = dout.shape
  dout_trans = dout.reshape(N, -1)
  dx_trans, dgamma_trans, dbeta_trans = batchnorm_backward(dout_trans, cache)
  dx = dx_trans.reshape(dout.shape)
  dgamma = np.array([dgamma_trans[i::C].sum() for i in xrange(C)])
  dbeta = np.array([dbeta_trans[i::C].sum() for i in xrange(C)])
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
